<template>
  <div class="schedule-info" v-if="state.isBig">
    ...
    <div>
      ...
      <button class="web-button-blue" style="margin-left:30px" type="button" @click="putSchedule" v-bind:disabled="title==''">수정</button>
    </div> 
</div>
...
<div class="mobile-schedule-main" v-else>
  ...
  <div>
      ...
      <button class="blueBtn" type="button" @click="putSchedule" v-bind:disabled="title==''">수정</button>
  </div> 
</div>
  ...
  setup() {
    ...
    const putSchedule = function() {
        const parList = state.participants.map( (par) => {
          ...
          if (parInfo) {
            // 원래 있는사람
            let existPar = initData.value.ModalDate.extendedProps.participant.find( (ep) => {
        ...
        axios
          .put('https://k5d105.p.ssafy.io:3030/groupCalendar/updateCalendar',
          {
            cid: state.cid,
          ...
          .then( (response) => {
            store.dispatch('deleteGroupCalendarData', initData.value.ModalDate.extendedProps.cid)
