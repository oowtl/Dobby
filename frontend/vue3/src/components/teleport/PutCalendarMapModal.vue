<template>
  <div class="modal" v-if="isOpen && isBig">
...
export default {
  ...
  setup() {
    ...
    onBeforeMount(() => {
      ...
      window.addEventListener('resize', handlePutMapModalWindowSize)
      handlePutMapModalWindowSize()
    })

    onUnmounted(() => {
      window.removeEventListener('resize', handlePutMapModalWindowSize)
    })
    ...
    const handlePutMapModalWindowSize = () => {
    ...
    const choiceWay = (course, way) => {
      ...
      store.dispatch('setPutMapChoice', {
    ...
    const state = reactive({
      ...
      latitude: 1.2,
      longitude: 1.3,
      goal: computed(() => store.state.putMapGoal),
      zoom: 15,
      curWay: [],
      curFootCourse: [],
      curDriveCourse: [],
