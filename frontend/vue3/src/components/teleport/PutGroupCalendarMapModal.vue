...
export default {
  ...
  setup() {
    ...
    onBeforeMount(() => {
      ...
      window.addEventListener('resize', handlePutGroupMapModalWindowSize)
      handlePutGroupMapModalWindowSize()
    })

    onUnmounted(() => {
      window.removeEventListener('resize', handlePutGroupMapModalWindowSize)
    })
    ...
    const handlePutGroupMapModalWindowSize = () => {
    ...
    const state = reactive({
      ...
      latitude: 1.2,
      longitude: 1.3,
      goal: computed(() => store.state.groupPutMapGoal),
      zoom: 15,
      curWay: [],
      curFootCourse: [],
      curDriveCourse: [],
